    ...
    class Program
    {
        ...
        static void DailyTemps()
        {
            const int LOW_TEMP = -20,  //constant for the min valid temp
                      ...
                      SENTINEL = 999;
            double highTemp,  //variable for the entered temp
            ...
            do
            {
                ...
                if (double.TryParse(ReadLine(), out highTemp))
                {
                    //if the temp is invalid then give an error and
                    //send user back to the top of the loop
                    //otherwise sum and increment
                    if (highTemp < LOW_TEMP || highTemp > HIGH_TEMP)
